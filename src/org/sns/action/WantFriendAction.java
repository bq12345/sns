 ...
 * @version Creation Time：2014-5-29 下午10:58:05 请求添加好友
 */
public class WantFriendAction implements Action {
	...
	public String execute() throws Exception {
		...
		if (SessionManager.isOnLine()) {
			...
			System.out.println(currentUser.getUsername() + "想要添加"
					+ stranger.getUsername() + "为好友");
			System.out.println("首先判断他们是不是好友+"
					+ friendService.isFriend(currentUser, stranger));

			// 判断是否是好友
			if (friendService.isFriend(currentUser, stranger)) {
				return "isFriend";
			} else {
				if (waitService.isWaited(currentUser, stranger)) {
